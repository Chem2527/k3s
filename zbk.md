...
This document provides the exact step-by-step execution guide for implementing PgBouncer connection pooling, Azure PostgreSQL server parameters, Azure DevOps variable group updates, and Azure Container Apps (ACA) compute autoscaling rules **specifically for the QA environment** (`zb-psql-pp-qa-eastus-001.postgres.database.azure.com`).

---

## 1. Master Infrastructure & Environment Details (QA)

| Infrastructure Component | Exact QA Resource / Value |
|---|---|
| **Azure Organization** | `ZenusBankInternational` |
| **Azure DevOps Project** | `ZB-CS - PP Digital Portal Solution` |
| **PostgreSQL QA Server** | `zb-psql-pp-qa-eastus-001.postgres.database.azure.com` |
| **Database Name & Schema** | Database: `fintech` | Schema: `fintech` |
...
| **Redis Cache QA Host** | `zb-pp-redis-qa-001.redis.cache.windows.net` (Port `6380`) |
| **Azure Resource Group** | `rg-purpleplum-qa-eastus` |
| **Log Analytics Workspace** | `log-zb-pp-fintech-qa-eastus-001` |
| **Application Insights** | `insight-zb-purpleplum-qa-eastus` |

---

## 2. Microservice Repositories & Variable Group Mapping (QA)

The following 8 microservice repositories and their associated Azure DevOps Variable Groups interact directly with the `fintech` schema on `zb-psql-pp-qa-eastus-001`:
...
#### Current Audited Variables in `ZB-CS - PP Digital Portal Solution`:
...
|---|---|---|---|---|
| `ZB-FintechUserManagment-QA` | `DB_PORT` | `5432` | **`6432`** | Routes `fintech_user` queries through PgBouncer pooler. |
| `ZB-FintechUserManagment-QA` | `DB_MAX_CLIENTS` | `20` | **`3`** | Caps client driver pool sockets (Math proof below). |
| `ZB-FintechSupeAdmin-QA` | `DB_PORT` | `5432` | **`6432`** | Routes `fintech_user` queries through PgBouncer. |
| `ZB-FintechSupeAdmin-QA` | `DB_MAX_CLIENTS` | `20` | **`3`** | Caps client driver pool sockets. |
| `ZB-FintechBusinessManagment-QA` | `DB_PORT` | `5432` | **`6432`** | Routes `fintech_user` queries through PgBouncer. |
| `ZB-FintechBusinessManagment-QA` | `DB_MAX_CLIENTS` | `20` | **`3`** | Caps client driver pool sockets. |
| `ZB-FintechBusinessSettings-QA` | `DB_PORT` | `5432` | **`6432`** | Routes `fintech_user` queries through PgBouncer. |
| `ZB-FintechBusinessSettings-QA` | `DB_MAX_CLIENTS` | `20` | **`3`** | Caps client driver pool sockets. |
| `ZB-FintechNotificationsManagement-QA` | `DB_PORT` | `5432` | **`6432`** | Routes `fintech_user` queries through PgBouncer. |
| `ZB-FintechNotificationsManagement-QA` | `DB_MAX_CLIENTS` | `20` | **`3`** | Caps client driver pool sockets. |
| `ZB-FintechDocumentsManagement-QA` | `DB_PORT` | `5432` | **`6432`** | Routes `fintech_user` queries through PgBouncer. |
...
* In PostgreSQL, an OLTP query (`SELECT`, `UPDATE` in `fintech` schema) takes **5 ms** to execute.
...
### C. Azure PostgreSQL Server Parameters (Server: `zb-psql-pp-qa-eastus-001`)
...
1. Open **Azure Portal** $\rightarrow$ **Azure Database for PostgreSQL Flexible Server** $\rightarrow$ `zb-psql-pp-qa-eastus-001`.
...
1. Open **Azure DevOps** $\rightarrow$ Project `ZB-CS - PP Digital Portal Solution` $\rightarrow$ **Pipelines** $\rightarrow$ **Library**.
...
Connect to QA PostgreSQL server (`zb-psql-pp-qa-eastus-001.postgres.database.azure.com`) via pgAdmin/psql with `fintech_user` and run the following **READ-ONLY** SQL script:

```sql
-- Verify PgBouncer & Defensive Parameter Settings on zb-psql-pp-qa-eastus-001
...
-- Verify Active Connections for fintech_user are routed via PgBouncer Port 6432
SELECT 
    state, 
    usename, 
    client_addr, 
